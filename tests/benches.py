...
from chiavdf import create_discriminant, prove_disc, evaluate_slow, prove_inter, verify_wesolowski, hash_int_both, hash_prime_both, evaluate
    ...
    for d in discriminant_sizes:
        ...
        for iters in iterations:
            ...
            for b in range(benches):
                ...
                tp1 = time.time()
                proof = prove_inter(discriminant,  initial_el, result_y_bis, inter, iters)
                tp2 = time.time()
            ...
            eval_time =  "{:.2E}".format(eval_time_mean) +  " ({:}%)".format(math.ceil(100 * eval_time_mean/ res_prove_mean ))
            eval_deviation = "{:.2E}".format(stdev(time_eval))
            
            prove_mean = sum(time_eval_prove)/len(time_eval_prove)
            proving_time =  "{:.2E}".format(prove_mean) +  " ({:}%)".format( math.ceil(100 *prove_mean / res_prove_mean) )
            ...
            verification_time = "{:.2E}".format(1000 * vf_mean) +  " ({:}%)".format( math.ceil(100* vf_mean / res_prove_mean))
            verification_deviation = "{:.2E}".format(1000 * stdev(res_verify))
            
            table.append([d, iters, ips, proveval_time, proveal_deviation, eval_time, eval_deviation, proving_time, proving_deviation, verification_time, verification_deviation ])
            ...
            table_prime.append([d, hash_time, hash_deviation, eval_time, eval_deviation ])
            
            hash_alpha_time = "{:.2E}".format(sum(time_alpha)/len(time_alpha))
            hash_alpha_deviation = "{:.2E}".format(stdev(time_alpha))
            eval_alpha_time = "{:.2E}".format(sum(time_alpha_eval)/len(time_alpha_eval))
            eval_alpha_deviation = "{:.2E}".format(stdev(time_alpha_eval))
            table_alpha.append([d, hash_alpha_time, hash_alpha_deviation, eval_alpha_time, eval_alpha_deviation ])
    headers = ["Size Discriminant", "#iters", "IPS", "EvalProve  (s)", "σ proving", "Eval (s)", "σ proving", "Prove (s)", "σ proving","Verification (ms)", "σ verification"]
    headers_eval = ["Size Discriminant", "Hash time (s)", "σ hashing", "Eval time (s)", "σ eval"]
    print()
